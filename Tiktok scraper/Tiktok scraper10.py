...
### issues (solved): รันช้าถ้ามีโพสที่ลบไปหรือซ่อนอยู่ เปิดทิคเก็ตไปแล้ว รอเค้าตอบ https://github.com/Russell-Newton/TikTokPy/issues/69
...
def get_video_stat(username, start_date, end_date):
    ...
    with TikTokAPI() as api:
            ...
            for video in videos_list:
                try:
                    ...
                    if requests_made >= ADJUSTED_RATE_LIMIT and elapsed_time < RATE_LIMIT_INTERVAL:
                        ...
                        time.sleep(0.0001)  # Introducing a small delay for safety
                    requests_made += 1
        ...
        # Calculate videos per total time spent
        total_seconds_elapsed = (minutes * 60) + seconds
        if total_seconds_elapsed == 0:
            vdo_per_min = total_videos
        else:
            vdo_per_sec = total_videos / total_seconds_elapsed
            vdo_per_min = vdo_per_sec * 60

        print(f"Total time of running: {int(minutes)} minutes and {int(seconds)} seconds | {vdo_per_min:.2f} post/min")
